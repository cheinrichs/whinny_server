...
router.post('/login_website', function (req, res, next) {
  ...
  knex('users').where('email', req.body.params.email.toLowerCase()).first().then(function (user) {
    if(!user){
...
router.get('/confirmCode/:user_phone/:confirmation_code', function (req, res, next) {
  ...
  'verified').where('phone', req.params.user_phone).first().then(function (user) {
    if(user){
      ...
      console.log(req.params.confirmation_code.toLowerCase());
      console.log(user.confirmation_code === req.params.confirmation_code.toLowerCase());
...
router.post('/markGroupMessagesAsRead', function (req, res, next) {
  knex('group_message_read_by').whereIn('group_message_read_id', req.body.newlyReadMessages).update({read: true, time_read: knex.fn.now()}).then(function () {
...
router.post('/createNewChat', function (req, res, next) {
  ...
  if(req.body.to_phone.length < 10){
    return res.json({ invalidNumberGiven: true });
  }
  knex('users').where('phone', req.body.to_phone).first().then(function (user) {
    if(user){
      ...
      knex('messages').insert({
        ...
        knex('user_action_log').insert({ user_id: req.body.from_user, action: 'Created a new chat with current whinny user ' + user.user_id, action_time: knex.fn.now() }).then(function () {
          res.json(user);
        })
      })
      ...
      //create a new user
      console.log("creating user", req.body.to_phone);
        ...
        knex('users').where('user_id', new_users[0].user_id).update({portrait_link: 'https://s3.amazonaws.com/whinnyphotos/profile_photos/' + new_users[0].user_id + '_PersonalProfilePic.jpg'}).then(function () {
          ...
          knex('broadcast_memberships').insert(memberships).then(function () {
            ...
            knex('messages').insert({
              ...
              knex('users').where('user_id', req.body.from_user).first().then(function (from_user) {
                if(from_user){
                  console.log('sending text message');
                  sendMms(req.body.to_phone, req.body.content, from_user.first_name, from_user.last_name);
                  new_users[0].textSent = true;
                  knex('user_action_log').insert({ user_id: req.body.from_user, action: 'Created a new chat with a user not registered to Whinny. A text was sent to ' + req.body.to_phone, action_time: knex.fn.now() }).then(function () {
                    res.json(new_users[0]);
                  })
                } else {
                  console.log("couldnt send text message");
                  new_users[0].textSent = false;
                  knex('user_action_log').insert({ user_id: req.body.from_user, action: 'Created a new chat with a user not registered to Whinny. A text was not sent to ' + req.body.to_phone, action_time: knex.fn.now() }).then(function () {
                    res.json(new_users[0]);
                  })
                }
              })
